...
async def on_startup():
    ...
    redis_conn = redis_async.from_url(os.getenv("REDIS_URL", "redis://localhost:6379/0"), encoding="utf8", decode_responses=True)
...
async def route_report(payload: ScanRequest, current_user: models.User = Depends(get_current_user)):
    """
    Generate a PDF report directly (bypasses Celery — report generation is fast, pure Python).
