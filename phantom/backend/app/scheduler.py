...
async def execute_scheduled_scan(target: str, modules_to_run: List[str], user_id: Optional[int] = None):
    ...
    Parameters
    ----------
    target         : Target domain/IP/URL to scan
    modules_to_run : List of module keys to execute
    user_id        : Optional user ID to associate with the scan session
    """
    ...
    async with async_session() as db:
        session_record = models.ScanSession(
            user_id=user_id,  # Can be None for system-triggered scans
            target=target,
            status="running",
    ...
    for m in modules_to_run:
        if m in MODULE_MAP:
            func = MODULE_MAP[m]
            # All tasks accept (target, options) — pass None for options in scheduled runs
    ...
    # Poll Celery tasks with a max timeout of 15 minutes
    ...
    async with async_session() as db:
        for m_name, task in zip(active_keys, tasks):
            ...
            if not task.ready():
                ...
                # If vulnerability detected, raise an Alert
                if vuln:
        ...
        # Update ScanSession status and overall risk
